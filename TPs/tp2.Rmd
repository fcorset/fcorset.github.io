...
```{r}
curve(dnorm,from = -4,to = 4,col="red",main="Densité de la loi Normale centrée réduite")
```
...
| Aire de    | Proba représentée | Ecriture avec | Valeur numérique |
| la figure  | par l'aire        |    $F(t)$     |    pnorm         |
|------------|-------------------|---------------|------------------|
|     1      |    $P(X<-1)$      |    $F(-1)$    |                  |
|     2      |                   |               |                  |
|     3      |                   |               |                  |
...
Quelle est la probabilité que $X\leqslant 17$ ? (utiliser la fonction *pnorm*) 
...
4. La taille des femmes françaises est modéliser par une loi gaussienne de moyenne $162$cm et de variance $144$. Quelle est la probabilité qu'un homme tiré au sort soit plus grand qu'une femme choisie au hasard ?
